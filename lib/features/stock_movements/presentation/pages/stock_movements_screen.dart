...
class StockMovementsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => MultiBlocProvider(
    providers: [
      BlocProvider(
        create: (_) => sl<StockMovementsCubit>()
          ..filters(
            productId: productId,
...
class _StockMovementsViewState extends State<_StockMovementsView> {
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(
      title: Text(context.l10n.stockMovements),
      actions: [
        IconButton(
          onPressed: () => context.read<StockMovementsCubit>().load(),
    ...
    body: Column(
      children: [
          ...
          child: ExpansionTile(
            ...
            children: [
                    ...
                    BlocBuilder<StockMovementsCubit, StockMovementsState>(
                      builder: (context, state) =>
                          DropdownButtonFormField<String>(
                            ...
                            onChanged: (value) =>
                                context.read<StockMovementsCubit>().filters(
                                  productId: value,
                                  type: state.type,
              ...
              BlocBuilder<StockMovementsCubit, StockMovementsState>(
                builder: (context, state) =>
                    DropdownButtonFormField<StockMovementType>(
                      ...
                      decoration: InputDecoration(
                        labelText: context.l10n.movementType,
                      ),
                      items: [
                        ...
                        ...StockMovementType.values
                            .where((type) => type != StockMovementType.unknown)
                            .map(
                              (type) => DropdownMenuItem(
                                value: type,
                                child: Text(_movementType(context, type)),
                              ),
                            ),
                      ],
                      onChanged: (value) =>
                ...
                children: [
                  Expanded(
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        labelText: context.l10n.purchaseInvoiceId,
                      ),
                    ),
                  ),
              ...
              BlocBuilder<StockMovementsCubit, StockMovementsState>(
                builder: (context, state) => Align(
                  ...
                  child: ActionChip(
                    ...
                    label: Text(context.l10n.dateRange),
                    onPressed: () async {
              ...
              Align(
                ...
                child: FilledButton(
                  onPressed: () {
                    final state = context.read<StockMovementsCubit>().state;
                    context.read<StockMovementsCubit>().filters(
                      productId: state.productId,
                      type: state.type,
                      purchaseId: _purchase.text.trim().isEmpty
                          ? null
                          : _purchase.text.trim(),
                      saleId: _sale.text.trim().isEmpty
                          ? null
                          : _sale.text.trim(),
                      from: state.from,
                      to: state.to,
        ...
        Padding(
          ...
          child: Text(
            context.l10n.movementHistoryNotice,
          ...
          child: BlocBuilder<StockMovementsCubit, StockMovementsState>(
            builder: (context, state) {
              ...
              if (state.status == StockMovementsStatus.failure &&
                ...
                return Center(
                  child: Column(
                    ...
                    children: [
                      Text(context.localizeFailure(state.failure)),
                      FilledButton(
                        onPressed: () =>
                            context.read<StockMovementsCubit>().load(),
              ...
              if (state.items.isEmpty) {
                return Center(child: Text(context.l10n.noStockMovements));
              }
              return RefreshIndicator(
                onRefresh: () => context.read<StockMovementsCubit>().load(),
                child: NotificationListener<ScrollNotification>(
                  onNotification: (n) {
                    if (n.metrics.extentAfter < 240) {
                      context.read<StockMovementsCubit>().load(replace: false);
                    }
                    return false;
                  },
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final movement = state.items[index];
                      final positive = movement.quantityDelta >= 0;
                      final date = movement.occurredAt ?? movement.createdAt;
                      final related =
                          movement.purchaseNumber ??
                          movement.saleReference ??
                      ...
                      return ListTile(
                        ...
                        subtitle: Text(
                          '${_movementType(context, movement.type)} · ${context.l10n.stockBeforeAfter(movement.stockBefore, movement.stockAfter)}\n${movement.reason ?? context.l10n.notAvailable}${related == null ? '' : '\n${context.l10n.reference}: $related'}${movement.createdByName == null ? '' : ' · ${movement.createdByName}'}',
                        ),
                        isThreeLine: true,
                        trailing: Text(
                          date == null
...
String _movementType(BuildContext context, StockMovementType type) =>
    switch (type) {
      StockMovementType.openingStock => context.l10n.openingStock,
      StockMovementType.purchase => context.l10n.purchaseMovement,
      StockMovementType.purchaseReversal => context.l10n.purchaseReversal,
      StockMovementType.sale => context.l10n.saleMovement,
      StockMovementType.saleReversal => context.l10n.saleReversal,
      StockMovementType.manualAdjustment => context.l10n.manualAdjustment,
      StockMovementType.unknown => context.l10n.unknownStatus,
    };
